...
*/
	{
		...
		rc = 0;
		if (!initialized) {
			if (!(hm = GetModuleHandle(CreateStdAccessibleObject_LIB))) hm = LoadLibrary(CreateStdAccessibleObject_LIB);
...
*/
	{
		...
		rc = 0;
		if (!initialized) {
			if (!(hm = GetModuleHandle(LresultFromObject_LIB))) hm = LoadLibrary(LresultFromObject_LIB);
