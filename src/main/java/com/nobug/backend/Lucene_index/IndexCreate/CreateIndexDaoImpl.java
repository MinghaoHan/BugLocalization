...
//        ResultSet tmpRS = stat.executeQuery("SELECT LAST_INSERT_ID()");
        ...
        for(int i=0;i<FNum;i++) {
            ...
            for(String tmpWord : wordCount.keySet()) {
                Double tfidf = queries.countTF(Path,tmpWord)*queries.countIDF(tmpWord);
                words.put(tmpWord,tfidf);
                Word = Word+ " " + tmpWord+ " "+String.valueOf(df.format(tfidf));
            }
            if(Word.length()>65280) {
                Word = "";
                //选择前 30 个词，写入数据库
